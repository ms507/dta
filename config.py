...
class RiskConfig:
    ...
    trailing_stop_pct: float = _env_float("TRAILING_STOP_PCT", 0.02)
...
class Config:
    ...
    min_signal_consensus: int = field(default_factory=lambda: _env_int("MIN_SIGNAL_CONSENSUS", 3))
    check_interval: int = field(default_factory=lambda: _env_int("CHECK_INTERVAL", 60))
    trading_cooldown_sec: int = field(default_factory=lambda: _env_int("TRADING_COOLDOWN_SEC", 1800))
    ...
    min_ai_buy_score: float = field(default_factory=lambda: _env_float("MIN_AI_BUY_SCORE", 0.10))
    max_ai_sell_score: float = field(default_factory=lambda: _env_float("MAX_AI_SELL_SCORE", -0.05))
