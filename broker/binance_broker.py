...
class BinanceBroker(BaseBroker):
    def __init__(
        ...
        logger.info(f"Binance broker ready ({mode}) | auth={auth_mode} | TLS verify={verify_mode}")
    ...
    def place_market_order(self, symbol: str, side: str, quantity: float) -> Order:
        ...
        try:
            result = self.client.order_market(symbol=symbol, side=side, quantity=quantity_str)
            logger.info(
